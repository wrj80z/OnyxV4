		...
		__index = function(self, index)
			return {
				async = function()
					...
					if index then
						local rank = bedwars.Client:Get(remotes.FetchRanks):CallServer({index.UserId})
...
run(function()
	...
	bedwars = setmetatable({
		KitSkinMeta = require(replicatedStorage.TS.games.bedwars["kit-skin"]["bedwars-kit-skin-meta"]).BedwarsKitSkinMeta,
	...
	local function ArtFunc()
		bedwars.Client:Get('VulcanArtilleryMark')
	end
	local function actgrave()
		bedwars.Client:Get('ActivateGravestone')
	end
	local function styxexitprotal()
		bedwars.Client:Get('StyxOpenExitPortalFromServer')
	end
	local function tryoepnexitportalfromclient()
		bedwars.Client:Get('StyxTryOpenExitPortalFromClient')
	end
	local function buyitemfunc()
		bedwars.Client:Get('BedwarsPurchaseItem')
	end
	local function buytierfunc()
		bedwars.Client:Get('RequestPurchaseTeamUpgrade')
	end
	local function turrentaimfunc()
		bedwars.Client:Get('AimTurret')
	end
	local function owlaimfunc()
		bedwars.Client:Get('OwlAimming')
	end
	local function owlprojectilefirefunc()
		bedwars.Client:Get('OwlFireProjectile')
	end
	local function BedBreakTriggerfunc()
		bedwars.Client:Get('BedBreakEffectTriggered')
	end
	local function collectEntityfunc()
		bedwars.Client:Get("CollectCollectableEntity")
	end
	local function styxspawnexpotportal()
		bedwars.Client:Get('StyxSpawnExitPortalFromServer')
	end
	local function SendFalconRequested()
		bedwars.Client:Get('SendFalconRequested')
	end
	local function ExtractFromDrillFunc()
		bedwars.Client:Get('ExtractFromDrill')
	end
	...
	local remoteNames = {
		...
		FetchRanks = Knit.Controllers.RankController.getRanks or function() end,
		ArtilleryMark = ArtFunc,
		SoulLink = Knit.Controllers.SoulBrokerController.onAbilityUsed or function() end,
		ActivateGravestone = actgrave,
		StyxPortal = Knit.Controllers.StyxKitController.tryUseEntrancePortal or function() end,
		StyxExitPortal = styxexitprotal,
		StyxSpawnExitPortal = styxspawnexpotportal,
		StyxSpawnEntrancePortal = Knit.Controllers.StyxKitController.onKitReplicationActivated or function() end,
	    TryOpenStyxPortalExit = Knit.Controllers.StyxKitController.tryOpenExitPortal or function() end,
		PurchaseTier = buytierfunc,
		PurchaseItem = buyitemfunc,
		AimTurret = AimTurret,
		OwlAim = owlaimfunc,
		OwlProjectileFire = owlprojectilefirefunc,
		CollectEntity = collectEntityfunc,
		BedBreakTrigger = BedBreakTriggerfunc,
		WinTrigger = Knit.Controllers.WinEffect.KnitStart or function() end,
		RecallFalcon = Knit.Controllers.FalconerController.onAbilityUsed or function() end,
		SendFalcon = SendFalconRequested,
		AttemptCardThrow = Knit.Controllers.FortunaController.onAbilityUsed or function() end,
		ExtractFromDrill = ExtractFromDrillFunc,
		MimicBlockPickPocketPlayer = getproto(Knit.Controllers.MimicController.onKitLocalActivated, 1) or function() end,
		FishCaught = Knit.Controllers.FishingRodEffectsController.KnitStart or function() end,
	}
...
run(function()
				...
				AimAssist:Clean(runService.PostSimulation:Connect(function(dt)
					if entitylib.isAlive then
						...
						if ent then
							...
							local cframe, speed = aimfuncs['Simple'](gameCamera.CFrame, ent, dt)
	...
	Mouse = AimAssist:CreateToggle({
		Name = 'Require mouse down'
	})
	StrafeIncrease = AimAssist:CreateToggle({
		Name = 'Strafe increase'
	})
	BlockBreak = AimAssist:CreateToggle({
									...
									if mouseinfo and mouseinfo.placementPosition == mouseinfo.placementPosition then
										local suc, res = pcall(function()
											return bedwars.placeBlock(({
												getPlacedBlock(mouseinfo.placementPosition)
											})[2])
										end)
										if not suc then
											warn(res)
			...
			if callback then
				...
				AutoClicker:Clean(inputService.InputEnded:Connect(function(input)
					if table.find({
						Enum.UserInputType.MouseButton1,
						Enum.UserInputType.Touch
					}, input.UserInputType) and Thread then
						task.cancel(Thread)
						Thread = nil
					end
				end))
				...
				bedwars.BlockBreaker.hitBlock = function(self, ...)
					...
					pcall(function()
						...
						if block and (not Blacklist.Enabled or not find(newlist, block.Name)) and (not BedCheck.Enabled or block.Name ~= 'bed') then
							bedwars.BlockBreakController.blockBreaker:setCooldown(Time.Value or 0.3)
						end
					end)
					return old(self, ...)
				end
...
run(function()
	...
	local function getAttackData()
        ...
        if AttackCheck.Enabled then
			...
			if (bedwars.SwordController.disableSwingState or false) then return false end
        end
        ...
        if LegitAura.Enabled then
            if (tick() - bedwars.SwordController.lastSwing) > math.clamp(ChargeTime.Value, 0.11, 0.3) then return false end
        end

        return sword, meta
    end
	local function calcPos(myPos, targetRoot)
		...
		local statsPing = statsService.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000
		local networkPing = lplr:GetNetworkPing()
		local ping = (statsPing * 0.5) + networkPing
		local velocity = targetRoot.AssemblyLinearVelocity
		local distance = (targetRoot.Position - myPos).Magnitude
		local speed = 250
		local travelTime = distance / speed
		local predictionTime = ping + travelTime
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if actualRoot then
									...
									if (v.Humanoid.FloorMaterial ~= Enum.Material.Air or math.random(1, 100) < AirChance.Value) or (not AirChanceToggle.Enabled) then
										local offset = Vector3.zero
										if calcPos then
											offset = calcPos(selfpos, actualRoot)
										end
										local data = {
											weapon = sword.tool,
											chargedAttack = {
												chargeRatio = store.attackReachUpdate - (tick() + lplr:GetNetworkPing())
											},
											lastSwingServerTimeDelta = bedwars.SwordController.lastAttack - (workspace:GetServerTimeNow() + lplr:GetNetworkPing()),
											...
											validate = {
												...
												targetPosition = {
													value = actualRoot.Position + offset
												},
												selfPosition = {
													value = pos
												}
										...
										if data.chargedAttack.chargeRatio < 0 then data.chargedAttack.chargeRatio = (-1*data.chargedAttack.chargeRatio) end
										if data.lastSwingServerTimeDelta < 0 then data.lastSwingServerTimeDelta = (-1*data.lastSwingServerTimeDelta) end
										AttackRemote:SendToServer(data)
									end
								end
							end
						end
	...
	AirChanceToggle = Killaura:CreateToggle({
		Name = 'Air Hit Toggle',
		Default = true,
		Function = function(callback)
			pcall(function()
				AirChance.Object.Visible = callback
			end)
		end
	})
	AirChance = Killaura:CreateSlider({
		...
		Suffix = '%',
		Darker = true,
		Visible = AirChanceToggle.Enabled
	})
	local methods = {
		'Damage',
		'Distance'
	...
	AttackCheck = Killaura:CreateToggle({
		Name = 'Attack Check',
		Tooltip = 'Checks if the sword can be swinged and possible to hit',
		Default = false
	})
...
run(function()
	...
	local function launchProjectile(item, pos, proj, speed, dir)
		...
		if projectileRemote:InvokeServer(item.tool, proj, proj, shootPosition.Position, pos, shootPosition.LookVector * speed, bedwars.IdUtil.generateId(8), {
			drawDurationSeconds = 1
...
run(function()
	...
	vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				old = clonefunction(modifier.addModifier)
				hookfunction(modifier.addModifier, function(...)
					local self, tab = ...
				...
				for i in modifier.modifiers do
					if (i.moveSpeedMultiplier or 1) < 1 then
						modifiers:removeModifier(i)
					end
				end
			else
				restorefunction(old, modifier.addModifier)
			end
		end
	})
end)
...
run(function()
	...
	local function getTarget()
		...
		if # plrs > 0 then
			for _, v in plrs do
				if not Targets.Walls.Enabled or not entitylib.Wallcheck(entitylib.character.Head.Position, v.RootPart.Position, {
					gameCamera,
					lplr.Character,
					v.Character
				}) then
						...
						if ent then
							...
							for _, data in getProjectiles() do
								...
								if (FireDelays[item.itemType] or 0) < tick() then
									...
									if calc then
										task.spawn(function()
											local dir, id = CFrame.lookAt(pos, calc).LookVector, bedwars.IdUtil.generateId(8)
											local shootPosition = (CFrame.new(pos, calc) * CFrame.new(Vector3.new(- bedwars.BowConstantsTable.RelX, - bedwars.BowConstantsTable.RelY, - bedwars.BowConstantsTable.RelZ))).Position
											projectileCooldown = 9e9
											local _, res = pcall(function()
												return projectileRemote:InvokeServer(
													item.tool, ammo, projectile, shootPosition, pos, dir * projSpeed, id, {
													drawDurationSeconds = 1,
													shotId = bedwars.IdUtil.generateId(8)
				...
				Spider:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive then
						...
						local ray = workspace:Raycast(
								root.Position - Vector3.new(0, entitylib.character.HipHeight - 0.5, 0), vec, rayCheck)
						if Active then
		...
		Normal = function(ent)
			...
			if nametag then
				...
				if Equipment.Enabled and store.inventories[ent.Player] then
					...
					nametag.Hand.Image = bedwars.getIcon(inventory.hand or {
						itemType = ''
					}, true)
					nametag.Helmet.Image = bedwars.getIcon(inventory.armor[4] or {
						itemType = ''
					}, true)
					nametag.Chestplate.Image = bedwars.getIcon(inventory.armor[5] or {
						itemType = ''
					}, true)
					nametag.Boots.Image = bedwars.getIcon(inventory.armor[6] or {
						itemType = ''
					}, true)
				...
				if Distance.Enabled then
					...
					if Sizes[ent] ~= mag then
						...
						local ize = getfontsize(
								removeTags(nametag.Text), nametag.TextSize, nametag.FontFace, Vector2.new(100000, 100000))
				...
				BulletTracers:Clean(workspace.ChildAdded:Connect(function(projectile)
					task.delay(0, function()
						...
						prediction.SpawnArcTracer(
								origin, velocityUnit, velocityMagnitude, gravity, travelTime, Curve.Value, {
	...
	AntiSuffocate = vape.Categories.Utility:CreateModule({
		...
		Function = function(call)
			if call then
				repeat
					if entitylib.isAlive then
						if getPlacedBlock(entitylib.character.RootPart.Position) and (
									getPlacedBlock(entitylib.character.RootPart.Position + Vector3.new(0, 2, 0)) and getPlacedBlock(entitylib.character.RootPart.Position - Vector3.new(0, 2, 0))) then
...
run(function()
	...
	local function firePearl(pos, spot, item)
		...
		if calc then
			...
			local projectile = bedwars.ProjectileController:createLocalProjectile(meta, 'telepearl', 'telepearl', pos, nil, dir, {
				drawDurationSeconds = 1
			})
			local res = projectileRemote:InvokeServer(
					item.tool, 'telepearl', 'telepearl', pos, pos, dir, bedwars.IdUtil.generateId(8), {
				drawDurationSeconds = 1,
				shotId = bedwars.IdUtil.generateId(8)
			}, workspace:GetServerTimeNow() - 0.045)
			task.spawn(function()
	...
	local function joinQueue()
		if not bedwars.Store:getState().Game.customMatch and bedwars.Store:getState().Party.leader.userId == lplr.UserId and bedwars.Store:getState().Party.queueState == 0 then
			if Random.Enabled then
				...
				local clonefunc = clonefunction(bedwars.QueueController.joinQueue)
				hookfunction(bedwars.QueueController.joinQueue, function(...)
					pcall(function()
						bedwars.Client:Get(remotes.AfkStatus):SendToServer({
							afk = false
						})
				...
				restorefunction(clonefunc, bedwars.QueueController.joinQueue)
			else
				if Bypass.Enabled then
					bedwars.Client:Get(remotes.AfkStatus):SendToServer({
						afk = false
...
run(function()
	...
	local function getAttackData()
		...
		local callback = meta.projectileType or function(res)
			return 'arrow'
		end
			...
			if proj then
				...
				if ent then
					calc = prediction.SolveTrajectory(
							selfpos, projSpeed, meta.gravitationalAcceleration or 196.2, Vector3.new(ent.RootPart.Velocity.X, 0, ent.RootPart.Velocity.Z), workspace.Gravity, ent.HipHeight, nil, RaycastParams.new(), nil, lplr:GetNetworkPing())
				end
				local dir = CFrame.lookAt(selfpos, calc).LookVector
				local shootPosition, id = (CFrame.new(selfpos, calc) * CFrame.new(Vector3.new(- bedwars.BowConstantsTable.RelX, - bedwars.BowConstantsTable.RelY, - bedwars.BowConstantsTable.RelZ))).Position, bedwars.IdUtil.generateId(8)
...
run(function()
	...
	local function staffFunction(plr, checktype)
		...
		StaffDetector:Clean(playersService.PlayerRemoving:Connect(function(prl)
			if not LeaveDetection.Enabled then
				return
			end
			if prl == plr then
				notif('StaffDetector', "361836499", 15, 'warning')
			end
		end))
	...
	LeaveDetection = StaffDetector:CreateToggle({
		...
		Tooltip = 'tells you whenever the staff has left the game'
	})
end)
...
run(function()
	...
	Switch = BedProtector:CreateToggle({
		Name = 'Auto Switch'
	})
	Smart = BedProtector:CreateToggle({
		Name = 'Smart',
		Default = true
	})
end)
...
run(function()
								...
								for _, v in chests do
									if (localPosition - v.Position).Magnitude <= Range.Value then
										if TeamCheck.Enabled then
											local chestTeamID = v:GetAttribute('Team') or 0
											local myteamID = lplr.Character:GetAttribute('Team') or -1
											if myteamID == chestTeamID then return end
										end
	...
	TeamCheck = ChestSteal:CreateToggle({
		Name = 'Team Check'
	})
end)
	...
	local function buyUpgrade(upgradeType, currencytable)
		...
		for i = currentTier, # upgrade.tiers do
			...
			if canBuy({
				...
				bedwars.Client:Get(remotes.PurchaseTier):CallServerAsync(upgradeType)
		...
		if entitylib.isAlive then
			...
			if SpeedPotion.Enabled and (not attribute or attribute == 'StatusEffect_speed') then
				...
				if speedpotion and (not lplr.Character:GetAttribute('StatusEffect_speed')) then
					if Limit.Enabled and not store.hand.tool.Name == 'speed_potion' then
						return
					end
					for _ = 1, 4 do
			...
			if Apple.Enabled and (not attribute or attribute:find('Health')) then
				if (lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) <= (Health.Value / 100) then
					...
					if apple then
						if Limit.Enabled and not store.hand.tool.Name == 'apple' then
							return
						end
			...
			if ShieldPotion.Enabled and (not attribute or attribute:find('Shield')) then
				if (lplr.Character:GetAttribute('Shield_POTION') or 0) == 0 then
					...
					if shield then
						if Limit.Enabled and not store.hand.tool.Name == 'big_shield' or not store.hand.tool.Name == 'mini_shield' then
							return
						end
						bedwars.Client:Get(remotes.ConsumeItem):CallServerAsync({
							item = shield.tool
			...
			if GodApple.Enabled and (not attribute or attribute:find('Health')) then
				if (lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) <= (Health.Value / 100) then
					...
					if apple then
						if Limit.Enabled and not store.hand.tool.Name == 'golden_apple' then
							return
						end
						bedwars.Client:Get(remotes.ConsumeItem):CallServerAsync({
							item = apple.tool
			...
			if Orange.Enabled and (not attribute or attribute:find('Health')) then
				if (lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) <= (Health.Value / 100) then
					...
					if orange then
						if Limit.Enabled and not store.hand.tool.Name == 'orange' then
							return
						end
						bedwars.Client:Get(remotes.ConsumeItem):CallServerAsync({
							item = orange.tool
...
run(function()
	...
	local MouseDown
	local Snow
	local ShowPath	local BlockHighlight
	...
	local function cachedIsBreakable(v)
		...
		local ok, result = pcall(bedwars.BlockController.isBlockBreakable, bedwars.BlockController, {
			blockPosition = blockPos
		}, lplr)
	...
	local function rebuildTeammateCache()
		local now = tick()
		if now - cachedTeammatesTime < 2 then
			return
		end
	...
	local function findPathBlock(targetPos, playerPos)
		...
		for i = step, distance - step, step do
			...
			checked[key] = true
			if (checkPos - targetPos).Magnitude < 2.5 then
				continue
			end
			if (checkPos - playerPos).Magnitude < 3 then
				continue
			end
			local block = getPlacedBlock(checkPos)
			if block then
				local ok, canBreak = pcall(bedwars.BlockController.isBlockBreakable, bedwars.BlockController, {
					blockPosition = checkPos / 3
				}, lplr)
	...
	local function attemptBreak(tab, localPosition, skipBreakCheck)
		...
		if MouseDown and MouseDown.Enabled and not inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
			return false
		end
		local best, bestDist = nil, math.huge
		for _, v in tab do
			local dist = (v.Position - localPosition).Magnitude
			if dist >= Range.Value or dist >= bestDist then
				continue
			end
		...
		Function = function(callback)
			...
			--	task.spawn(hookFreezeController)
				...
				local function trackAdd(obj)
					if not _trackedNames[obj.Name] then
						return
					end
...
run(function()
	...
	BedAlarm = vape.Categories.Minigames:CreateModule({
		Name = 'Bed Alarm',
		Function = function(callback)
			if callback then
	...
	local Checks = {
		Killaura = function()
			...
			HackerDetector:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				if not HackerDetector.Options['Killaura'].Enabled then
					return
				end
				if damageTable.damageType == 0 and damageTable.fromEntity then
					...
					if from and from ~= lplr then
						...
						if lastHit <= 0.26 + lplr:GetNetworkPing() then
							Strikes[from] = (Strikes[from] or 0) + 1
							task.delay(60 + statsService.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000, function()
								pcall(function()
									Strikes[from] -= 1
								end)
							end)
							if Strikes[from] > 4 then
								Added(from, 'Killaura')
							end
						end
		...
		Reach = function()
			HackerDetector:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				if not HackerDetector.Options['Reach'].Enabled then
					return
				end
				if damageTable.damageType == 0 and damageTable.fromEntity then
					...
					if player and player ~= lplr then
						...
						local reach = math.floor(meta and meta.attackRange or 12.4) + 4 + lplr:GetNetworkPing() + statsService.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000
		...
		Fly = function()
			task.spawn(function()
				repeat
					if not HackerDetector.Options['Fly'].Enabled then
						return
					end
					for _, ent in playersService:GetPlayers() do
						if not CheatersFlagged[ent] and ent ~= lplr then
							local char = ent.Character
		...
		FastBreak = function()
			local MineData = {}
			local Strikes = {}
			HackerDetector:Clean(vapeEvents.BreakBlockEvent.Event:Connect(function(self)
				if not HackerDetector.Options['FastBreak'].Enabled then
					return
				end
				local player = self.player
				if player then
					local lastMine = (os.clock() - (MineData[player] or 0))
					if lastMine < 0.013 + lplr:GetNetworkPing() then
						Strikes[player] = (Strikes[player] or 0) + 1
						task.delay(45 + statsService.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000, function()
							pcall(function()
								Strikes[player] -= 1
							end)
						end)
						if Strikes[player] > 2 then
							Added(player, 'Fastbreak')
						end
					end
				end
				MineData[player] = os.time()
			end))
		end,
		Velocity = function()
			local Strikes = {}
			HackerDetector:Clean(vapeEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				if not HackerDetector.Options['Velocity'].Enabled then
					return
				end
				if damageTable.damageType == 0 and damageTable.entityInstance then
					local target = playersService:GetPlayerFromCharacter(damageTable.entityInstance)
					if target and target ~= lplr then
						local targetRoot = damageTable.entityInstance:FindFirstChild("HumanoidRootPart")
						local fromRoot = damageTable.fromEntity and damageTable.fromEntity:FindFirstChild("HumanoidRootPart")
						if not targetRoot or not fromRoot then return end

						local startPos = targetRoot.Position
						local direction = (startPos - fromRoot.Position)
						direction = Vector3.new(direction.X, 0, direction.Z).Unit

						local expectedVelocity = bedwars.KnockbackUtil.calculateKnockbackVelocity(direction, 50, {
							vertical = 0,
							horizontal = 1
						})
						local expectedHorizontal = Vector3.new(expectedVelocity.X, 0, expectedVelocity.Z).Magnitude

						task.delay(0.1 + lplr:GetNetworkPing(), function()
							if not target.Character then return end
							local root = target.Character:FindFirstChild("HumanoidRootPart")
							if not root then return end

							local velocity = root.AssemblyLinearVelocity
							local actualHorizontal = Vector3.new(velocity.X, 0, velocity.Z).Magnitude

							if expectedHorizontal > 8 then
								local ratio = actualHorizontal / expectedHorizontal

								if ratio < 0.25 then
									Strikes[target] = (Strikes[target] or 0) + 1
									task.delay(60 + statsService.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000, function()
										pcall(function()
											Strikes[target] -= 1
										end)
									end)
									if Strikes[target] > 3 then
										Added(target, 'Velocity')
									end
								end
							end
						end)
		...
		Function = function(callback)
			if callback then
				ArmorTrims:Clean(entitylib.Events.LocalAdded:Connect(function(ent)
					task.delay(1, function()
						if not ArmorTrims.Enabled then
							return
						end
		...
		Function = function(callback)
			if callback then
				BedBreakEffect:Clean(vapeEvents.BedwarsBedBreak.Event:Connect(function(data)
					firesignal(bedwars.Client:Get(remotes.BedBreakTrigger).instance.OnClientEvent, {
						player = data.player,
...
run(function()
	vape.Legit:CreateModule({
		...
		Function = function(callback)
			if callback then
				local old = clonefunction(bedwars.WindWalkerController.spawnOrb)
				getgenv().oldcleankit = old
				...
				restorefunction(getgenv().oldcleankit or function()
				end, bedwars.WindWalkerController.spawnOrb)
			end
		end,
	...
	local function Added(active)
		...
		if replacements[active.statusEffect] then
			...
			if meta and (meta.image or meta.item) then
				effectimage.Image = meta.image or bedwars.getIcon({
					itemType = meta.item
				}, true)
			end
		end
		...
		Function = function(callback)
			if callback then
				WinEffect:Clean(vapeEvents.MatchEndEvent.Event:Connect(function()
					for i, v in getconnections(bedwars.Client:Get(remotes.WinTrigger).instance.OnClientEvent) do
						if v.Function then
							v.Function({
			...
			if callback then
				...
				local function onLaunchTriggered(child)
    				...
    				task.delay(0.05, function()
    					for i = 1, 2 do
    						task.spawn(bedwars.breakBlock, child, false, nil, true, nil, AS.Enabled)
    					end
    				end)
					if AJ.Enabled then
	...
	AutoKitFunctions = {
		miner = function()
			kitCollection('petrified-player',function(v)
				if v:GetAttribute('Team') == lplr.Character:GetAttribute('Team') then return end
				if Legit.Enabled then
					local miner_mine_stone = bedwars.GameAnimationUtil:playAnimation(lplr, bedwars.AnimationType.MINER_MINE_STONE)
					bedwars.MinerController:setupMineAnimationEffects(lplr, miner_mine_stone, lplr.Character)
					task.delay(2.5 + lplr:GetNetworkPing(), function()
				...
				repeat
					if not Legit.Enabled or isHoldingItem({'vacuum'}) then
						for _, obj in collectionService:GetTagged('ghost') do
							...
							if id then
								bedwars.Client:Get(remotes.CollectEntity):SendToServer({id=id})
							end
						end
					end
					task.wait(0.1)
		...
		falconer = function()
			...
			local thread = task.spawn(function()
				repeat
					...
					if plr and bedwars.AbilityController:canUseAbility('ACTIVATE_FALCON_INDICATOR') then
						local pos = plr.RootPart.Position
						bedwars.AbilityController:useAbility('ACTIVATE_FALCON_INDICATOR')
						task.wait(0.05)
						bedwars.AbilityController:useAbility('SEND_FALCON', newproxy(true), {target=pos})
						bedwars.Client:Get(remotes.SendFalcon):SendToServer({strikeZoneEpicenter=pos})
						canRecall = true
						task.wait(3)
					elseif bedwars.AbilityController:canUseAbility('RECALL_FALCON') then
						if canRecall then
							canRecall = false
							bedwars.Client:Get(remotes.RecallFalcon):SendToServer()
						end
					end
		...
		rebellion_leader = function()
			...
			local thread = task.spawn(function()
				repeat
					...
					if health <= 40 and bedwars.AbilityController:canUseAbility("rebellion_shield") then
						bedwars.AbilityController:useAbility("rebellion_shield")
						task.wait(0.05)
						continue
					end
					if plr then
						if getCurrentAura() ~= "damage" and bedwars.AbilityController:canUseAbility("rebellion_aura_swap")  then
							bedwars.AbilityController:useAbility("rebellion_aura_swap")
							task.wait(0.1)
						end
					else
						if getCurrentAura() ~= "healing" and bedwars.AbilityController:canUseAbility("rebellion_aura_swap")  then
							bedwars.AbilityController:useAbility("rebellion_aura_swap")
							task.wait(0.1)
						end
					end
		...
		sorcerer = function()
			local thread = task.spawn(function()
				kitCollection('alchemy_crystal', function(v)
					...
					bedwars.Client:Get(remotes.CollectEntity):SendToServer({id = v:GetAttribute("Id"),collectableName = v.Name})
			...
			bedwars.LaunchPadController.attemptLaunch = function(...)
				...
				if (workspace:GetServerTimeNow() - self.lastLaunch) < 0.4 then
					if block:GetAttribute('PlacedByUserId') == lplr.UserId and (block.Position - entitylib.character.RootPart.Position).Magnitude < 30 then
						task.delay(0.05, function()
							for i = 1, 2 do
								task.spawn(bedwars.breakBlock, block, false, nil, true, Legit.Enabled or false, nil)
							end
						end)
					end
				end
		...
		necromancer = function()
			...
			local thread = task.spawn(function() 
				kitCollection('Gravestone', function(v)
					local suc, res = pcall(function()
						bedwars.Client:Get(remotes.ActivateGravestone):CallServer({
							skeletonData = {
					...
					if ok and result and result.success then
						pcall(function()
							bedwars.NecromancerController:useGravestone(lplr, v)
						end)
					end
				end, r, false)
			end)
					...
					if entitylib.isAlive then
						...
						for _, drill in ipairs(drills) do
							if drill:GetAttribute("PlacedByUserId") == userId then
								pcall(function()
									bedwars.Client:Get(remotes.ExtractFromDrill):SendToServer({ drill = drill })
								end)
							end
						end
					end
		...
		hannah = function()
			...
			local thread = task.spawn(function()
				kitCollection('HannahExecuteInteraction', function(victim)
					local success = bedwars.Client:Get(remotes.HannahPromptTrigger):CallServer({
						user = lplr,
						victimEntity = victim
					})
					if success then
						...
						if v.Targetable and v.Character and v.Player then
							...
							if distance <= (Legit.Enabled and 12 or 30) then
								if collectionService:HasTag(v.Character, "MimicBLockPickPocketPlayer") then
									pcall(function()
										if bedwars.Client:Get(remotes.MimicBlockPickPocketPlayer):CallServer(v.Player) then
				...
				repeat
					if entitylib.isAlive and bedwars.VoidDragonController and bedwars.VoidDragonController.inDragonForm then
						...
						if target and target.RootPart then
							...
							if shouldFire then
								bedwars.Client:Get(remotes.DragonBreath):SendToServer({player=player})
								task.wait(1) 
							end
						end
					end
				...
				repeat
					...
					if bedwars.AbilityController:canUseAbility('CARD_THROW') then
						...
						if plr then
							...
							task.wait(0.1)
							pcall(function()
								bedwars.Client:Get(remotes.AttemptCardThrow):SendToServer({targetEntityInstance = plr.Character})
							end)
							task.wait(0.5)
						end
					end
        ...
        summoner = function()
            repeat
                local plr = entitylib.EntityPosition({
                    Range = Legit.Enabled and 20 or 32,
                    Part = 'RootPart',
                    Players = true,
                ...
                if plr then
                    ...
                    localPosition += shootDir * math.max((localPosition - plr.RootPart.Position).Magnitude - 16.399, 0)
					local ct = workspace:GetServerTimeNow()
                    bedwars.Client:Get(remotes.SummonerClawAttack):SendToServer({
                        position = localPosition,
                        direction = shootDir,
                        clientTime = ct
                    })
                    bedwars.SummonerClawHandController.lastAttackTime = ct + lplr:GetNetworkPing()
					if Legit.Enabled then
                    	bedwars.SummonerClawController:clawAttack(lplr, localPosition, shootDir, store.hand.tool.Name)
					end
                end
    
                task.wait(bedwars.SummonerKitBalance.CLAW_COOLDOWN)
            until not AutoKit.Enabled
        end,
			...
			local thread = task.spawn(function()
				kitCollection('ElementTome', function(v)
					...
					if secret then
						...
						local result = bedwars.Client:Get(remotes.MageSelect):SendToServer({secret = secret})
				...
				repeat
					...
					if plr and plr.Character and linkCount < 8 then
						...
						if not linkedTargets[char] then
							local suc, res = pcall(function()
								bedwars.AbilityController:useAbility('soul_link')
							end)
							task.wait(lplr:GetNetworkPing())
 							bedwars.Client:Get(remotes.SoulLink):CallServerAsync(char):andThen(function(p12)
								if not p12.result then end
		...
		berserker = function()
			local function getBedNear()
				local localPosition = entitylib.isAlive and entitylib.character.RootPart.Position or Vector3.zero
				for _, v in collectionService:GetTagged('bed') do
					if (localPosition - v.Position).Magnitude < (Legit.Enabled and 10 or 32) + lplr:GetNetworkPing() and not v:GetAttribute('Team' .. (lplr:GetAttribute('Team') or - 1) .. 'NoBreak') then
						return v
					end
				end
				return nil
			...
			repeat
				local bed = getBedNear()
				if bed and bedwars.AbilityController:canUseAbility('berserker_rage') then
		...
		yeti = function()
			local function getBedNear()
				local localPosition = entitylib.isAlive and entitylib.character.RootPart.Position or Vector3.zero
				for _, v in collectionService:GetTagged('bed') do
					if (localPosition - v.Position).Magnitude < (Legit.Enabled and 8 or 12) + lplr:GetNetworkPing() and not v:GetAttribute('Team' .. (lplr:GetAttribute('Team') or - 1) .. 'NoBreak') then
						return v
					end
				end
				return nil
			...
			repeat
				local bed = getBedNear()
				if bed and bedwars.AbilityController:canUseAbility('yeti_glacial_roar') then
		...
		cowgirl = function()
			...
			local charging = nil
			local lasso_swinging = nil
			pcall(function()
				projectileRemote = bedwars.Client:Get(remotes.FireProjectile).instance
			end)
			rayCheck.FilterDescendantsInstances = {
				workspace:FindFirstChild('Map', 20)
			}
			repeat
				local plrs = entitylib.AllPosition({
					Range = Legit.Enabled and 12 or 25,
					Part = 'RootPart',
					Players = true,
					NPCs = true,
					Limit = 1,
					Wallcheck = Legit.Enabled,
					Sort = Legit.Enabled and sortmethods.Angle or sortmethods.Distance
				})
				if #plrs > 0 then
					for _, ent in plrs do
						local function getLasso()
							for i, v in store.inventory.inventory.items do
								if v.itemType == 'lasso' then
									return v.tool
								end
							end
							return nil
						end
						local MaxAngle = Legit.Enabled and 60 or 360
						local root = entitylib.character.RootPart
						local delta = (ent.RootPart.Position - root.Position)
						local localfacing = root.CFrame.LookVector * Vector3.new(1, 0, 1)
						local angle = math.acos(localfacing:Dot((delta * Vector3.new(1, 0, 1)).Unit))
						...
						if IsOnCoolDown then
							task.wait(1 + lplr:GetNetworkPing())
							continue
						end
						if not Legit.Enabled then
							...
							if not store.hand.tool or store.hand.tool.Name ~= 'lasso' then
								task.wait(lplr:GetNetworkPing())
								continue
							end
						end
						...
						if calc then
							task.spawn(function()
								local dir, id = CFrame.lookAt(pos, calc).LookVector, bedwars.IdUtil.generateId(8)
								local shotpos = (CFrame.new(pos, calc) * CFrame.new(Vector3.new(-bedwars.BowConstantsTable.RelX, -bedwars.BowConstantsTable.RelY, -bedwars.BowConstantsTable.RelZ))).Position
								charging = nil
								lasso_swinging = nil
								if not getgenv().charging then
									charging = bedwars.GameAnimationUtil:playAnimation(lplr, bedwars.AnimationType.LASSO_CHARGE, {
										looped = true
									})
									getgenv().charging = true 
									local LASSO_SWING = bedwars.SoundList.LASSO_SWING
									local kitsin = bedwars.KitSkinMeta[bedwars.KitSkinController:getKitSkin(lplr)]
									if kitsin and kitsin.lassy then
										LASSO_SWING = kitsin.lassy.lassoSwingSound
									end
									lasso_swinging = bedwars.SoundManager:playSound(LASSO_SWING, {
										looped = true
									})
								end
								task.wait((Legit.Enabled and 2 or 0.125) + ps)
								local _, res = pcall(function()
									return projectileRemote:InvokeServer(
										getLasso(), nil, 'lasso', shotpos, pos, dir * projSpeed, id, {
											drawDurationSeconds = (Legit.Enabled and 0.5 or 1) + lplr:GetNetworkPing() + ps,
											shotId = bedwars.IdUtil.generateId(8)
										}
									)
								end)
								getgenv().charging = false
								if charging then charging:Stop() end
								if lasso_swinging then lasso_swinging:Stop() end
								IsOnCoolDown = true
								if not res then
									task.wait(lplr:GetNetworkPing() + ps)
									IsOnCoolDown = false
								else
									local LASSO_THROW = bedwars.SoundList.LASSO_THROW
									local kitskin = bedwars.KitSkinMeta[bedwars.KitSkinController:getKitSkin(lplr)]
									if kitskin and kitskin.lassy then
										LASSO_THROW = kitskin.lassy.lassoThrowSound
									end
									bedwars.SoundManager:playSound(LASSO_THROW)
									task.delay(8 + ps, function()
										IsOnCoolDown = false
									end)
								end
							end)
				...
				hookfunction(bedwars.CannonHandController.launchSelf, function(...)
					...
					if AutoBreak.Enabled then
						if (cannon.Position - entitylib.character.RootPart.Position).Magnitude <= 30 then
							task.delay(0.05, function()
								for i = 1, 2 do
									task.spawn(bedwars.breakBlock, cannon, false, nil, true, AutoSwitch.Enabled or false, nil)
								end
							end)
						end
					end
					...
					if entitylib.isAlive then
						...
						if myOwl then
							...
							if ent then
								...
								task.wait(Delay.Value - lplr:GetNetworkPing())
								bedwars.Client:Get(remotes.OwlAim):SendToServer({
									owl = myOwl.Part,
									starting = true
								})
								bedwars.Client:Get(remotes.OwlProjectileFire):SendToServer({
									ProjectileRefId = bedwars.IdUtil.generateId(8),
									direction = dir,
									fromPosition = og,
									initialVelocity = dir,
								})
					...
					if turret then
                        ...
                        if ent then
                            ...
                            if pos then
								...
								bedwars.Client:Get(remotes.AimTurret):SendToServer({
...
run(function()
	...
	AntiLasso = vape.Categories.Kits:CreateModule({
		Name = 'Anti Lasso',
		Tooltip = 'Freezes you whenever you are lassoed.',
		Function = function(callback)
			AntiLasso:Clean(lplr.Character.DescendantAdded:Connect(function(v)
				local lassoed = v:IsA('Accessory') and v:FindFirstChild('Rope') 
				if lassoed then
					if rng(os.clock()):NextNumber(0,100) <= Chance.Value then
						local ent = (Target.Enabled and entitylib.AllPosition({
							Range = 50,
							Part = 'RootPart',
							Players = true,
			...
			AntiLasso:Clean(lplr.Character.DescendantRemoving:Connect(function(v)
				local lassoed = v:IsA('Accessory') and v:FindFirstChild('Rope') 
				if lassoed then
	...
	Target = AntiLasso:CreateToggle({Name='Only when targetting'})
end)
